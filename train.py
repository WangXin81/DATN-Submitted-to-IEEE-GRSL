...
def DATNll(input_list, ad_net_list, grl_layer_list, class_weight, use_gpu=True):
    loss = 0
    ...
    for i in range(config["num_iterations"]):
        if i % config["test_interval"] == 0:
            ...
            temp_acc = image_classification_test(dset_loaders, \
                    base_network, test_10crop=prep_config["test_10crop"], gpu=use_gpu)
        ...
        entropy_loss = loss_params["entropy_trade_off"] * EntropyLoss(nn.Softmax(dim=1)(outputs))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DATN')
    parser.add_argument('--gpu_id', type=str, nargs='?', default='0', help="device id to run")
    parser.add_argument('--net', type=str, default='ResNet50', help="Options: ResNet18,34,50,101,152; AlexNet")
    parser.add_argument('--dset', type=str, default='N_A', help="The dataset or source dataset used")
    parser.add_argument('--s_dset_path', type=str, default='../data/N_A/N_A_19_list.txt', help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='../data/Dt-UCM/Dt-UCM_list.txt', help="The target dataset path list")
    parser.add_argument('--test_interval', type=int, default=500, help="interval of two continuous test phase")
    parser.add_argument('--snapshot_interval', type=int, default=5000, help="interval of two continuous output model")
    parser.add_argument('--output_dir', type=str, default='datn', help="output directory of our model (in ../snapshot directory)")
    ...
    if config["dataset"] == "N_A":
        config["data"] = {"source":{"list_path":args.s_dset_path, "batch_size":36}, \
                          "target":{"list_path":args.t_dset_path, "batch_size":36}, \
                          "test":{"list_path":args.t_dset_path, "batch_size":4}}
        config["class_num"] = 19
    elif config["dataset"] == "N_A22":
        config["data"] = {"source":{"list_path":args.s_dset_path, "batch_size":36}, \
                          "target":{"list_path":args.t_dset_path, "batch_size":36}, \
                          "test":{"list_path":args.t_dset_path, "batch_size":4}}
